...
<head>
    ...
    <title>{{$type}} Customers Report</title>
</head>
<style>
    @page {
        header: page-header;
...
<body>
    ...
    <htmlpagefooter name="page-footer">
        <table width="100%">
            <tr>
                <td width="70%" align="left" style="border: 0px solid white;">{PAGENO}/{nbpg} [ {{$type}} Customers Report ] : {{date('d M, Y')}}</td>
    ...
    <div>
        <div>
            <table class="table">
                <thead>
                <tr><td colspan="6" align="center"><h3>{{$type}} Customers Report</h3></td></tr>
                    <tr>
                        <th>Name</th>
                        <th>Type</th>
                        <th>Phone</th>
                ...
                <tbody>
                    ...
                    @php($type_info = DB::table('customer_types')->where('id', $customer->customers_type_id)->first())
                    <tr>
                        <td>{{$customer->name}}</td>
                        <td>{{optional($type_info)->type_name}}</td>
                        <td>{{$customer->phone}}</td>
                        <td>{{$customer->code}}</td>
                        <td>{{optional($customer)->address}}</td>
                        <td>{{optional($customer)->balance}} {{$currency}}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
