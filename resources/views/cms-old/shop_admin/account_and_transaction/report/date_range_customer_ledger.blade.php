...
<body>
    ...
    <htmlpagefooter name="page-footer">
        <table width="100%">
            <tr>
                <td width="70%" align="left" style="border: 0px solid white;">{PAGENO}/{nbpg} [ {{date("d-m-Y", strtotime($start_date))}} To {{date("d-m-Y", strtotime($end_date))}}, Ledger Summery Of {{$customer_info->name}} ]</td>
...
<br>
            <ul style="width: 100%; float:left;">
                <li style="width: 49%;">
                    <table class="table table-bordered">
                        <thead>
                        <tr><td align="center" colspan="3"><h3>Payment Received</h3></td></tr>
                            <tr>
                                <th>Date</th>
                                <th>Amount</th>
                                <th>Voucher Num.</th>
                        ...
                        <tbody>
                            ...
                            <tr><td>{{date('d-m-Y', strtotime($payment->created_at))}}</td><td>{{number_format($payment->received_amount, 2)}}</td><td>#{{str_replace("_","/", $payment->voucher_number)}}</td></tr>
                            @php( $total_payment = $total_payment + $payment->received_amount)
                            @endforeach
                            <tr>
                ...
                <li style="width: 49%;">
                    <table class="table table-bordered">
                        <thead>
                        <tr><td colspan="4" align="center"><h3>Sell Info</h3></td></tr>
                            <tr>
                                <th>Date</th>
                                <th>Inv Total</th>
                                <th>Instant Paid</th>
                        ...
                        <tbody>
                            @php( $invoice_total = 0)
                            @php( $instant_paid_total = 0)
                            
                            @foreach($invoices as $order)
                            <tr>
                                <td>{{date('d-m-Y', strtotime($order->date))}}</td>
                                <td>{{number_format($order->invoice_total-$order->pre_due, 2)}}</td>
                                <td>{{number_format($order->paid_amount, 2)}}</td>
                                <td>#{{str_replace("_","/", $order->invoice_id)}}</td>
                            </tr>
                            @php( $invoice_total = $invoice_total+$order->invoice_total-$order->pre_due)
                            @php( $instant_paid_total = $instant_paid_total + $order->paid_amount);
                            @endforeach
                            <tr>
                                <td align="left" colspan="2">Total Sell = {{number_format($invoice_total, 2)}}</td>
                                <td align="left" colspan="2">Instant Paid = {{number_format($instant_paid_total, 2)}}</td>
    ...
    <div>
            <table class="table">
                ...
                <tbody>
                    @php( $total_refund = 0)
                    @foreach($return_invoices as $return_order)
                    <tr>
                        <td>{{date('d-m-Y', strtotime($return_order->date))}}</td>
                        <td>#{{str_replace("_","/", $return_order->invoice_id)}}</td>
                        <td>{{$return_order->return_current_times}}</td>
                        <td>{{number_format($return_order->refundAbleAmount, 2)}}</td>
                    </tr>
                    @php( $total_refund = $total_refund+$return_order->refundAbleAmount)
                    @endforeach
                    <tr>
